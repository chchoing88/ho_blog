...
__개발자의 실수에 의해 발생되는 예외__ 들은 __미확인 예외__ 들이다. 즉, 컴파일러가 체크되지 않는 예외를 말한다. 예를 들어 0으로 나눠서 에러가 발생한다거나 배열범위 등이 있겠다. 이런것들은 개발자가 조금만 더 신경쓰면 예외가 발생하지 않는 예외들이다. 
...
checked 예외(`확인된 예외`) 는 컴파일 단계에서 확인되며 반드시 처리해야 하는 예외입니다.
