...
> 빠른 요약: 프런트 엔드 테스트를 작성 도중에 많은 함정을 발견한다. 요약하면, 유지관리가 힘들어지고 실행 시간이 느려지며 최악의 경우 신뢰할 수 없는 테스트가 발생할 수 있다. 하지만 이 글에서는 경험상 개발자들이 저지르는 흔한 실수들에 대해 그리고 어떻게 피할 수 있는지에 관해서 이야기 하겠다. 테스트는 더는 고통 서러운 게 아니다.
...
이 기사에서 필자는 경험했던 함정들에 대해 말할 것이다. 그중 일부는 필자 잘못이 있었다. 이 글에서 동일한 환경과 상황이 같을 수는 없다. 필자는 단위 테스트를 위한 Jest 프레임워크를 사용하고 end-to-end 테스트를 위한 Cypress 프레임워크를 사용함으로써 일상 업무에 큰 영향을 받았다. 필자는 필자의 분석이 최대한 추상적으로 유지될 수 있도록 할 것이다. 그래야 다른 프레임워크를 사용하는 사용자들에게 조언을 할 수 있을 것이다. 만약 불가능하다고 생각되는 부분이 있다면 이야기 나눌 수 있도록 아래 댓글 작성을 부탁한다. 일부 예제는 단위, 통합 또는 end-to-end 테스트 등 모든 테스트 유형에 적용될 수 있을 것이다.
## 프론트엔드 테스트 함정
...
* **유닛 테스트**는 애플리케이션에 작은 단위를 확인한다. 이 유닛들은 클래스, 인터페이스 또는 메서드일 수 있다. 유닛 테스트에서는 사전 정의된 입력을 사용하여 기대 출력을 제공하는지를 확인한다. 따라서 테스트 단위는 개별적으로 격리하여 시행합니다.
...
이 모든 작업을 함께 수행하면 우리의 애플리케이션을 제공에 대한 확신을 얻을 수 있다. 프런트 엔드 테스트를 통해 원하는 대로 사용자가 UI와 상호작용을 할 수 있다. 또 다른 관점에서 이런 테스트 실행을 통해 많은 리소스와 에너지를 소모하는 수동 테스트 없이도 오류 없는 애플리케이션 출시를 보장할 수 있다.

하지만 다양한 원인을 가진 문제들로 인해 이 가치 있는 일이 보이지 않을 수 있다. 이런 문제들이 함정이라고 생각될 수 있다. 최선의 의도로 무언가를 하고 있지만 결국 고통스럽고 지치는 걸 상상해 봐라. 이것은 더 나쁜 종류의 기술 부채일 것이다.
...
필자가 가장 좋아하는 테스트 원칙은 중복, DRY(반복하지 마라) 원칙이다. 만약 추상화가 테스트를 이해하는 데 방해된다면 중복 코드를 모두 피해라.
...
이러한 문서는 미래의 자신과 팀이 테스트를 더 잘 이해하는 데 도움이 되므로 이 경우 필수적일 수 있다. 프로덕션 코드에 대한 일부 모범 사례는 테스트 코드에 적합하지 않다. 테스트는 단순히 프로덕션 코드가 아니며, 우리는 절대 그렇게 취급해서는 안 된다. 물론 테스트 코드는 생산 코드와 동일하게 취급해야 한다. 그러나 일부 규약과 모범 사례는 맞지 않을 수 있다. 이러면 골든 룰을 기억하고 개발자 경험을 우선시해야 한다.

## 테스트 설계의 함정

이 섹션의 첫 번째 몇 가지 예에서는 먼저 테스트 트랩에 빠지지 않는 방법에 관해 설명하겠다. 그 후에 필자는 테스트 디자인에 관해 이야기하겠다. 만약 당신이 이미 오랜 프로젝트를 하고 있다면, 이것은 여전히 유용할 것이다.

### 3가지 규칙

다음 예제코드를 보자. 테스트의 타이틀에 주목하자. 테스트의 콘텐츠 자체는 주된 것이 아니다.
...
위 테스트를 보고 첫눈에 그것이 무엇을 테스트할 것인지 알 수 있는가? 특히 테스트 결과에서 이 제목을 살펴본다고 상상해 보자. (예를 들면 지속적인 통합에서 파이프라인의 로그 항목을 확인할 수 있습니다) 분명 에러가 발생할 것이다. 하지만 그게 무슨 에러일까? 어떤 상황에서 에러가 나야 할까? 알다시피, 첫눈에 이 테스트가 달성하고자 하는 의도를 이해하기는 쉽지 않다. 왜냐하면 제목이 의미가 없기 때문이다.
...
1. 무엇을 테스트하고 있습니까? 이때는 프로퍼티를 테스트한다.
...
그렇다면 축하한다! 당신은 정보 처리 속도가 놀라울 정도로 빠른 것이다. 그렇지 않으면 걱정하지 마라. 테스트 구조가 크게 개선될 수 있기 때문에 이는 매우 정상적이다. 예를 들어 선언문과 주장은 구조에 전혀 관심을 두지 않고 작성되고 혼합되었다. 어떻게 하면 이 시험을 개선할 수 있을까?
...
하지만, 그들은 예상치 못한 일들을 생각해야 할인지도 모른다. 두 사람이 테스트를 나타내고 종이가 테스트 데이터를 나타내는 이 이미지를 적용해보자. 이 두 테스트의 이름을 A 테스트와 B 테스트로 하겠다. 창의적이지 않은가? 요점은 테스트 A와 테스트 B가 동일한 테스트 데이터를 공유하거나 더 나쁜 경우에는 이전 테스트에 의존한다는 것이다.

이것은 불안정한 테스트로 이어지기 때문에 문제가 된다. 예를 들어, 이전 테스트가 실패하거나 공유 테스트 데이터가 손상된 경우 테스트 자체를 성공적으로 실행할 수 없다. 또 다른 시나리오는 테스트가 랜덤 순서로 실행되는 것이다. 이 경우 이전 테스트가 해당 순서로 유지될지 아니면 다른 순서로 완료될지 예측할 수 없다. 그래서 테스트 A와 B의 기준을 읽게 된다. 이는 end-to-end 테스트에 국한되지 않는다. 보통 유닛 테스트는 두 테스트가 동일한 기준 데이터를 변화시킨다.

좋아. 필자 일상 업무에서 나온 end-to-end 테스트 코드 예제를 보자. 다음 테스트에서는 온라인 상점의 로그인 기능에 관해 설명한다.
...
두 번째 단계는 테스트를 실행하는 데 필요한 모든 데이터를 생성하는 것이다. 이 예제 코드에서는 매장에 로그인할 수 있는 고객을 만들어야 한다. 테스트 자체에 맞게 테스트에 필요한 모든 데이터를 만들고 싶다. 이렇게 하면 테스트는 독립적이며 실행 순서는 무작위로 지정할 수 있다. 요약하자면, 두 단계 모두 테스트가 다른 테스트 또는 외부의 부작용으로부터 격리되어 결과적으로 안정성을 유지하는 데 필수 적이다.

## 구현 함정

좋아. 테스트 디자인에 관해 얘기했다. 하지만 좋은 테스트 디자인을 말하는 것만으로는 충분치 않다. 왜냐하면 불가사의한 요소가 세부사항 속에 숨어있기 때문이다. 그러니 우리 테스트를 살펴보고 실제 구현을 해보자.
...
선택자를 보았는가? CSS 선택자이다. 음, 글쎄, 당신은 궁금해할 것이다. "왜 그것들이 문제가 됩니까? 독특하고 취급 및 유지 관리가 쉽고 완벽하게 사용할 수 있는데!” 그러나 항상 그런다고 확신하는가?
...
테스트에 구현 세부사항을 사용하는 것보다 더 나은 대안이 있다고 생각한다. 대신, **사용자가 알아차릴 수 있는 것들을 테스트해보자.** 더 나은 방법은 변경 가능성이 적은 선택자를 선택하는 것입니다. 필자가 가장 좋아하는 선택자 유형은 데이터 속성이다. 개발자는 리팩토링 중에 데이터 속성을 변경할 가능성이 작아 테스트에서 요소를 찾기에 완벽하다. 소스 코드로 작업하는 개발자에게 명확하게 목적을 전달할 수 있도록 **의미 있는 방식으로 이름을 지정할 것**을 권장한다. 다음과 같이 할 수 있다.
