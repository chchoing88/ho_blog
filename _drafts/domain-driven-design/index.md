...
- 도메인에 위치해야 할 중요 규칙 구현 코드가 사용자 인터페이스까지 끌려나오게 만드는 주범이다.
- UI는 사용자의 입력을 받고 출력을 보여주는 것 맡은 바 책임이다.
- 비즈니스 로직이 UI에 구현되어서는 안 된다.
- UI에는 최소한의 로직만 존재해야 한다. UI가 똑똑해질수록 수정 비용이 높아지며 숮어 작업도 괴로워 진다.
...
- 계층형 아키텍처를 구성하는 4개의 계층은 다음과 같다.
  ...
  - 도메인 계층
  - 인프라스트럭처 계층
- 도메인 계층은 소프트웨어가 적용될 분야에서 문제 해결에 필요한 지식을 담는 계층이다.
- 애플리케이션 계층은 도메인 계층의 요소를 지휘하는 요소가 모인 계층이다. 여기에는 애플리케이션 서비스가 있다.
- 애플리케이션 서비스는 도메인 객체의 직접적인 클라이언트가 되며 유스케이스를 구현하는 진행자 역할을 한다.
- 프리젠테이션 계층은 사용자 인터페이스와 애플리케이션이 연결되는 곳이다. 이 층의 주요 책임은 출력의 표시와 입력의 해석이다.
- 인프라스트럭처 계층은 자신 외의 계층을 지탱하는 기술적 기반에 대한 접근을 제공한다.(메세지 송신, 퍼시스턴시 제공 모듈)
- 이 아키텍쳐의 원칙은 위에서 아래를 향한다는 점이다. 상위에 있는 계층은 자신보다 하위에 있는 계층에 의존할 수 있다.
- MVC 프레임워크 컨트롤러는 사용자의 입력 데이터인 HTTP 요청을 애플리케이션에 전달할 수 있게 변환하는 역할을 하므로 프레젠테이션 계층에 걸맞는 요소다.
- 애플리케이션 서비스는 자신 보다 하위에 있는 도메인 계층과 인프라스트럭처 계층에 의존한다.
- 애플리케이션 서비스는 문제를 해결하기 위해 도메인 객체의 과업을 진행하고 관리하는 것이다. 도메인 규칙이나 동작을 직접 구현해서는 안된다.
- 인프라스트럭처 계층에는 도메인 객체를 지원하는 기술적 기능 외에도 애플리케이션 계층이나 프레젠테이션 계층에 기술적 기능을 제공하는 객체도 포함된다.

### 헥사고날 아키텍쳐

- 이 아키텍처의 콘셉트는 애플리케이션과 그 외 인터페이스나 저장 매체를 자유롭게 탈착 가능하게 하는 것이다.
- 가장 적합한 예시는 게임기 이다. 컨트롤러를 순정이 아니라 서드파티를 이용해도 되고 모니터 와 저장매체 역시 포트만 맞으면 그 어느것을 사용해도 된다.
- 애플리케이션 외의 모듈은 마치 게임 컨트롤러처럼 다른 것으로 바꿔 끼울 수 있는 요소다.
- 헥사고날 아키텍처는 어댑터가 포트 모양만 맞으면 동작하는 것과 같다고 해서 포트앤어댑터(port-and-adapters) 라고 부르기도 한다.
- 계층형 아키텍처와의 차이점은 인터페이스를 이용해 의존관계를 관리한다는 점이다.
- 계층형 아키텍처에서는 논리적 계층 분리만 되어 있을 뿐 인터페이스를 사용할지 여부는 강제되지 않는다.

### 클린 아키텍쳐

- 클릭 아키텍쳐 요점은 비즈니스 규칙을 캡슐화한 모듈을 중심에 두는 콘셉트다.
- 클린 아키텍처에서 말하는 엔티티는 비즈니스 규칙을 '캡슐화한 객체' 혹은 '데이터 구조와 함수를 묶은 것'을 가리키는 것으로 굳이 따지면 우리가 배운 도메인 객체에 가까운 개념이다.
- 클린 아키텍처는 사용자 인터페이스나 데이터스토어 같은 세부사항은 가장자리로 밀어내고, 의존관계의 방향을 안쪽을 향하게 함으로써 세부사항이 추상에 의존하는 의존관계 역전 원칙을 달성한다.
- 헥사고날 아키텍처와 클린 아키텍처의 가장 큰 차이점은 구현 내용이 언급되는지 여부에 있다.
- 헥사고날 아키텍처에서는 포트와 어댑터를 이용해 탈착이 가능하게 하라는 원칙만 있었다.
- 클린 아키텍처에는 콘셉트를 실현하기 위해 구체적인 구현 방식이 명시된다.

```uml
Presenter --|> IUseCaseOutputPort
UseCaseInteractor --> IUseCaseOutputPort
UseCaseInteractor --|> IUseCaseInputPort
Controller --> IUseCaseInputPort
```

- 사용자 input을 Controller 가 받으면 컨트롤러는 IUseCaseInputPort 를 구현한 Use Case(구현체)에게 일을 시킨다.
- Use Case는 IUseCaseInputPort 를 구현한다.
- Use Case 는 해당 결과물을 내보내기 위해 IUseCaseOutputPort 를 구현한 Presenter(구현체)에게 일을 시킨다.
- Presenter는 IUseCaseOutputPort를 구현한다.
- IUseCaseInputPort의 클라이언트는 Controller 이다. 또한 IUseCaseOutputPort의 클라이언트는 UseCaseInteractor 이다.
- IUseCaseInputPort나 IUseCaseOutputPort를 구현한 스텁을 만들어서 테스트 하기 용이하다.
- 클린 아키텍처의 콘셉트에서 가장 중요한 것은 비즈니스 모듈을 캡슐화한 모듈을 중심에 두고 의존관계의 방향을 절대적으로 통제하는 데 있다.
- 모든 세부사항이 도메인에 의존하게 하면 소프트웨어의 주도권을 중요도가 높은 도메인이 쥐게 할 수 있다.
