...
border-image-repeat을 사용하면 가장자리 이미지 처리 방법을 바꿀 수 있다.
...
이례적으로 border-image-slice, border-image-width, border-image-outset 프로퍼티 값을 모두 제공하는 동시에 각 값이 어느 프로퍼티 값인지 명시하기 위해서는 이들을 슬래시로 구분하고 순서 또한 slice, width, outset 순서를 지키도록 정해졌다. 이 세 값은 정확하게 순서를 지켜야 하지만 border-image-source와 border-image-repeat은 원하는 위치에 써도 된다.
